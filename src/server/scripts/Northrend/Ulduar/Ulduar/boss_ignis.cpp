                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_END_POT:
                            ...
                            {
                                slagPotTarget->ExitVehicle();
								if(slagPotTarget->GetTypeId() == TYPEID_PLAYER)
								{
							AchievementEntry const* Horneado = sAchievementStore.LookupEntry(RAID_MODE(2927, 2928)); //Horneado 10 y 25 jugadores
                            ((Player*)slagPotTarget)->CompletedAchievement(Horneado);  
								}
                                slagPotTarget = NULL;
                                _slagPotGUID = 0;
