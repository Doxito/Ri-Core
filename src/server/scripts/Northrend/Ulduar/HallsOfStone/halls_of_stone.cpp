...
#define GOSSIP_ITEM_START               "Brann, te ayudaremos."
#define GOSSIP_ITEM_PROGRESS            "Suficientes lecciones de Historia, Brann. Vamos a pasar."
...
void MovimientoAdd(Creature* target)
{
	target->SetReactState(REACT_AGGRESSIVE);
	target->AI()->DoZoneInCombat();
	target->SetSpeed(MOVE_RUN, 1.5f, true);
    target->AddThreat(me, 1700.0f);
    //target->GetMotionMaster()->MovementExpired();   
	//target->GetMotionMaster()->MovePoint(0, 943.977f, 379.257f, 207.425f);
	target->GetMotionMaster()->MoveChase(me);
        ...
        void SpawnSummon(uint32 uiType)
        {
            ...
            switch (uiType)
            {
                case 1:
                {
                    ...
                    if (Creature* mod = me->SummonCreature(CREATURE_DARK_RUNE_PROTECTOR, SpawnLocations[rnd], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000))
                    MovimientoAdd(mod);
                    if (Creature* mod =  me->SummonCreature(CREATURE_DARK_RUNE_STORMCALLER, SpawnLocations[rnd], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000))
				    MovimientoAdd(mod);
                    break;
                }
                case 2:
                    for (uint8 i = 0; i < 2; ++i)
					   ...
					   MovimientoAdd(mod);
						   break;
                case 3:
                   if (Creature* mod = me->SummonCreature(CREATURE_IRON_GOLEM_CUSTODIAN, SpawnLocations[rnd], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000))
                    MovimientoAdd(mod);
					   break;
                case 4:
